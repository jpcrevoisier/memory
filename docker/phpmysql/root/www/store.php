...
{	// déclaration des propriétés de l'objet avec trois types visibilités
	...
	private function sendFeedBack(array $feedback, string $header=null)
	{
	...
	private function mysqliConnect()
	{
		...
		&&
			getenv('mysql_pass')!==false
		&&
			getenv('mysql_base')!==false
		)
		{
			$this->hostname=getenv('mysql_host');
			$this->username=getenv('mysql_user');
			$this->password=getenv('mysql_pass');
			$this->database=getenv('mysql_base');	
		}
		...
		elseif(file_exists('mysqli.inc.php'))
		{
			$mysqliConfig=include('mysqli.inc.php');
	...
	private function selectDatas()
	{
		...
		$query="select `pseudo`, `score`, `grid` from `".$this->database."`.`memory` order by `score` desc limit 25;";
